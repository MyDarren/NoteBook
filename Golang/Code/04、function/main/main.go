...
函数基本语法
 ...
 3、函数参数传递方式
	...
	不管值传递还是引用传递，传递给函数的都是变量的副本，不同的是，值传递的是值的拷贝，引用传递的是地址的拷贝
	一般来说，地址拷贝的效率高，数据量小，而值拷贝取决于拷贝的数据大小，数据越大，效率越低
	函数中的变量是局部的，函数外不生效
 	基本数据类型和数组都是默认值传递的，即进行值拷贝，在函数内修改，不会影响到原来的值
	如果希望函数内的变量能修改函数外的变量，可以传入变量的地址&，函数内以指针的方式操作变量
 4、变量的作用域
	 在函数内部声明/定义的变量叫局部变量，作用域仅限于函数内部
	 在函数外部声明/定义的变量叫全局变量，作用域在整个包中都有效，如果变量名首字母大写，则在整个程序中有效
	 如果变量是在一个代码块中，那么这个变量的作用域就在该代码块中
 5、golang函数不支持重载
 6、在golang中，函数也是一种数据类型
