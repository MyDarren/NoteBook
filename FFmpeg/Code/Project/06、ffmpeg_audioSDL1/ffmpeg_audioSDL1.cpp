...
#define MAX_AUDIO_FRAME_SIZE 19200  //1 second of 48kHZ 32bit audio
...
int main(int argc,char *argv[]){
    ...
    if (!codec) {
        av_log(NULL,AV_LOG_ERROR,"cannot find decode!");
        return -1;
    }
    ...
    while (av_read_frame(formatContext,packet) >= 0){
        if (packet->stream_index == audioIndex) {
            ...
            if (result < 0) {
                av_log(NULL,AV_LOG_ERROR,"fail to decode audio!");
                return -1;
            }
            if (gotPicture > 0) {
                swr_convert(
    ...
    */
    ...
    wanted_spec.silence = 0;
    //播放AAC，M4A时out_linesize为4096，此时为1024
    //播放MP3，WMA时out_linesize为4608，此时为1152
    wanted_spec.samples = codecContext->frame_size;
    wanted_spec.callback = fill_Audio;
    wanted_spec.userdata = codecContext;
    ...
    while (av_read_frame(formatContext,packet) >= 0){
        if (packet->stream_index == audioIndex) {
            ...
            if (gotPicture > 0) {
                ...
                printf("index:%5d\tpts:%lld\tpacketSize:%d\tout_linesize:%d\tframeSize:%d\tsampleRate:%d\tout_buffer_size:%d\n",
                        index,packet->pts,packet->size,
                        out_linesize,codecContext->frame_size,
                        codecContext->sample_rate,out_buffer_size
                );
                fwrite(out_Buffer,1,out_buffer_size,pcmFile);
                index ++;
            }
