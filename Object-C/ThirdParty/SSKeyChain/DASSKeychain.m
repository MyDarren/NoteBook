...
NSString *const DAkSSKeychainErrorDomain = @"com.samsoffes.sskeychain";

NSString *const DAkSSKeychainAccountKey = @"daacct";
NSString *const DAkSSKeychainCreatedAtKey = @"dacdat";
NSString *const DAkSSKeychainClassKey = @"dalabl";
NSString *const DAkSSKeychainDescriptionKey = @"dadesc";
NSString *const DAkSSKeychainLabelKey = @"dalabl";
NSString *const DAkSSKeychainLastModifiedKey = @"damdat";
NSString *const DAkSSKeychainWhereKey = @"dasvce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef DASSKeychainAccessibilityType = NULL;
#endif

@interface DASSKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation DASSKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = DASSKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:DAkSSKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (DASSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return DASSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (DASSKeychainAccessibilityType) {
		CFRelease(DASSKeychainAccessibilityType);
	}
	DASSKeychainAccessibilityType = accessibilityType;
}
#endif
