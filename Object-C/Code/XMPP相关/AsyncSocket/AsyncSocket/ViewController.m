...
@interface ViewController ()<NSStreamDelegate,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,GCDAsyncSocketDelegate>
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    [self sendDataWithString:sendString tag:102];
    return YES;
}
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag{
    ...
    if (tag == 101) {  //登录指令
        ...
        //[self performSelectorOnMainThread:@selector(addData:) withObject:receiveString waitUntilDone:YES];
        [self.messageArray addObject:receiveString];
        [self.tableView reloadData];
    }
}

- (void)addData:(NSString *)receiveString{
    [self.messageArray addObject:receiveString];
    [self.tableView reloadData];
}
...
- (void)sendDataWithString:(NSString *)string tag:(long)tag{
...
- (IBAction)loginAction:(id)sender {
    ...
    [self sendDataWithString:loginStr tag:101];
}
